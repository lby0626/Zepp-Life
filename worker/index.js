...
// 从 GitHub Pages 页面发来的请求，CORS 白名单
const ALLOWED_ORIGINS = ['https://zjy1020.github.io', 'https://lby0626.github.io'];

// 硬编码的 data_json 模板（来自 main.py）
...
/**
 * AES-128-CBC 加密（匹配 Python encrypt_login_data）
 * key/IV 固定值，与 main.py 一致
 */
async function encryptLoginData(plainBytes) {
  const key = await crypto.subtle.importKey(
    'raw', AES_KEY, { name: 'AES-CBC' }, false, ['encrypt']
  );
  const encrypted = await crypto.subtle.encrypt(
    { name: 'AES-CBC', iv: AES_IV }, key, plainBytes
  );
...
async function handleUpdate(user, password, steps) {
  ...
  const queryStr = buildQuery(loginData);
  const plainBytes = new TextEncoder().encode(queryStr);
  const encrypted = await encryptLoginData(plainBytes);
  log.push(`登录参数加密完成，长度:${encrypted.length}`);
  ...
  const userId = r2json.token_info.user_id;
  log.push('登录第二步成功，login_token 与 userid 获取完成');
  ...
  async fetch(request) {
    ...
    if (request.method === 'POST' && url.pathname === '/api/update') {
      try {
        ...
        if (!user || !password) {
          return new Response(
            JSON.stringify({ success: false, message: '账号和密码不能为空' }),
            { status: 400, headers: { 'Content-Type': 'application/json', ...corsHeaders } }
          );
        }

        const steps = parseInt(rawSteps);
        if (isNaN(steps) || steps < 1 || steps > 98800) {
          return new Response(
            JSON.stringify({ success: false, message: '步数范围: 1~98800' }),
            { status: 400, headers: { 'Content-Type': 'application/json', ...corsHeaders } }
          );
        }
        ...
        return new Response(
          JSON.stringify({ success: false, message: err.message, log: '' }),
